...
//                True value for window.CST_OVERRIDES.tcUser indicates user is a teacher candidate.
...
// 10.07.2019 tps Reinstate so we can use for past semesters with restored access students.
//                Assume that obs.js has been reloaded already.

// Wait for DOM to load before trying to read dashboard framework data
document.addEventListener("DOMContentLoaded", initOverridesGlobals);

function initOverridesGlobals() {
  // Retrieve terms data from hidden page element & make it globally available to scripts
  window.CST_OVERRIDES = {};
  window.CST_OVERRIDES.semesters = JSON.parse(document.getElementById('semesters').innerText);
  window.CST_OVERRIDES.facultyUser = JSON.parse(document.getElementById('facultyUser').innerText);
  window.CST_OVERRIDES.appLocation = document.getElementById('appLocation').innerText;
  window.CST_OVERRIDES.canvasBaseUrl = document.getElementById('canvasBaseUrl').innerText;
  window.CST_OVERRIDES.tcUser = !!document.getElementById('tcUser');
...
function populatePage() {
  ...
  const hiddenObs =  window.CST_OVERRIDES.dashUser.hiddenObs;

  for (semester of window.CST_OVERRIDES.semesters) {
  ...
  window.CST_OVERRIDES.dashUser.hiddenObs = validAssignmentIds;

  // Retrieve CritiqueIt data by course, to speed up page.
  for (semester of window.CST_OVERRIDES.semesters) {
    if (semester.isupe_assignment_style === 'overrides') {

      let iSupeIds = semester.term_courses.map( e => e.iSupe_course_id );
      iSupeIds = iSupeIds.filter( (value, index, self) => self.indexOf(value) === index);
      for (iSupeId of iSupeIds) {
        populateISupeCourse(iSupeId, semester.term_courses, function() {});
      }
...
function hideAssignment(courseId, assignmentId) {
  ...
  let hiddenObs = window.CST_OVERRIDES.dashUser.hiddenObs.slice();
      ...
      for (let j = 0; j <= lastDataColIndex; ++j) {
        ...
        if (container && (container.id === targetContainerId)) {
          ...
          setTimeout(function() {
            ...
            hiddenObs = window.CST_OVERRIDES.dashUser.hiddenObs;

            // Use a deep copy of the terms collection because we're going 
            // to mess with the assignment lists.
            let term = null;
            const semesters = JSON.parse(document.getElementById('semesters').innerText);
            for (let semester of semesters) {
              let targetTerm = semester.term_courses.find( e => e.course_id === courseId);
              if (targetTerm) {
                term = {...targetTerm};
                break;
              }
...
function createObsDiv(courseId, assignment, isEyeToggle) {
  ...
  */ 
  ...
  if (isEyeToggle) {  // Render eye in show all view.
    ...
    const isHidden = window.CST_OVERRIDES.dashUser.hiddenObs.includes(assignment.id);
    ...
    eyeImage.src = window.CST_OVERRIDES.appLocation + 'obs-icons/eye-seen-48.png';
  ...
  if (!window.CST.tcUser) {
    ...
    editImage.src = window.CST_OVERRIDES.appLocation + 'obs-icons/edit-icon.png';
...
function getCritiqueItStatus(courseId, assignment, done) {
  ...
  httpRequest.open('GET', window.CST_OVERRIDES.appLocation + 'api/v0/critiqueit/assignments/' + assignmentId, true);
  httpRequest.send();
...
*/
function populateISupeCourse(iSupeCourseId, terms, done) {
  ...
  httpRequest.onreadystatechange = getCritiqueItStatusHandler;
  httpRequest.open('GET', window.CST_OVERRIDES.appLocation + 'api/v0/critiqueit/courses/' + iSupeCourseId + '/assignments', true);
  httpRequest.send();
  ...
  function getCritiqueItStatusHandler() {
    if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
        ...
        // const iSupeTerms = window.CST_OVERRIDES.terms.filter( e => e.iSupe_course_id === iSupeCourseId);
        const iSupeTerms = terms.filter( e => e.iSupe_course_id === iSupeCourseId);
...
function addDefaultStageIcons(parentElement) {
  ...
  function appendDefaultStageIcons(baseImgFileName) {
    ...
    const src = window.CST_OVERRIDES.appLocation 
      + 'obs-icons/' 
...
function addHandRaised(parentElement, isHandRaised, isObservationOpen) {
  ...
  const handRaisedSrc = window.CST_OVERRIDES.appLocation 
    + 'obs-icons/hand_' 
    + (isHandRaised ? '' : 'not_')
    + 'raised' 
    + (isObservationOpen ? '' : '_final')
...
function populateFinalStatus(term) {
  ...
  for(let student of term.students) {
    ...
    getOverrideSubmissionsByStudent(courseId, iSupeCourseId, iSupeSectionId, studentId, assignmentId, function() {});
  }
}


function getOverrideSubmissionsByStudent(courseId, iSupeCourseId, sectionId, studentId, assignmentId, done) {
  ...
  httpRequest.open('GET', window.CST_OVERRIDES.appLocation + 'api/v0/sections/' + sectionId + '/students/' + studentId + '/submissions', true);
  httpRequest.send();
...
function submitAdd() {  
  ...
  httpRequest.open('POST', window.CST_OVERRIDES.appLocation + 'api/v0/courses/' + courseId + '/assignments', true);
  ...
  function addAssignmentHandler() {
    if ((httpRequest.readyState === XMLHttpRequest.DONE) && (httpRequest.status === 200)) {
      ...
      // Adjust display of the course whose assignment have now changed.
      let termCourse = findTermCourseForISupeCourse(courseId);
      if (termCourse) {
        let termCourseId = termCourse.course_id;
...
//       // for (let term of window.CST_OVERRIDES.terms) {
//         // let term = findTermWithCourse(courseId);
        
//       let student = findStudentByISupeCourseSection(courseId, sectionId);
//       if (student) {
//         let termCourseId = term.course_id;
...
function submitEdit() {  
  ...
  httpRequest.open('PUT', window.CST_OVERRIDES.appLocation + 'api/v0/courses/' + courseId + '/assignments/' + assignmentId, true);
  httpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  httpRequest.send(postString);
  ...
  function editAssignmentHandler() {
    if ((httpRequest.readyState === XMLHttpRequest.DONE) && (httpRequest.status === 200)) {
      ...
      if (updatedAss) {
        ...
        // Find the corresponding term course.
        let termCourse = findTermCourseForISupeCourse(courseId);
        if (termCourse) {
          const termCourseId = termCourse.course_id;

          // Find student object for student with restored access in iSupervision course 
          // with the assignment override that got edited.
          ...
          } // end loop through course's students
        } // end if found course with assignment update
...
function saveAndTestNumber(dataObj, done) {
  ...
  httpRequest.open('POST', window.CST_OVERRIDES.appLocation + 'api/v0/dashuser/' + window.CST_OVERRIDES.facultyUser.id + '/smsSaveAndTest', true);
  httpRequest.setRequestHeader("Content-type", "application/json");
...
function submitPhone() {
  ...
  if (isValid) {
    ...
    var submitData = {
      ...
      userName: window.CST_OVERRIDES.facultyUser.name   // Include user's name in test message
    }
...
function showCreateGDocForm(iSupeCourseId, assignment, docName) {
  ...
  // 10.18.2019 tps Only search courses using assignment overrides
  var candidateName = '';
  let term = findTermCourseForISupeCourse(iSupeCourseId);
  if (term) {
  ...
  // var candidateName = '';
  // for(var term of window.CST_OVERRIDES.terms) {
  //   for (var student of term.students) {
  //     var foundAssignment = student.assignment_overrides.find( e => e.id === assignment.id);
  //     if (foundAssignment) {
  //       candidateName = student.name;
...
function submitCreateGDoc() {  
  ...
  httpRequest.open('POST', window.CST_OVERRIDES.appLocation + 'api/v0/google/drive/doc', true);
  httpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  httpRequest.send(postString);
...
// Find the semester term object representing a specific course ID
...
// Find the term course object for an iSupervision course ID, for a semester with section override assignments
function findTermCourseForISupeCourse(iSupeCourseId) {
  let termCourse = null;
  const semesters = window.CST_OVERRIDES.semesters.filter( e => e.isupe_assignment_style === 'overrides');
  for (let semester of semesters) {
    termCourse = semester.term_courses.find( e => e.iSupe_course_id === iSupeCourseId);
    if (termCourse) {
      break; // We found it and we're done.
    }
  }
  return termCourse;
}
